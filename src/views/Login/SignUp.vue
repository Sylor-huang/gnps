      ...
      <el-card>
        ...
        >
          <div class="text-center mb-8">
            <div class="text-2xl">{{ $t("pld.signup") }}</div>
          </div>
          ...
          <el-form-item prop="confirm_password" :label="$t('common.cpasswd')">
            <el-input
        ...
        <div class="text-right mt-8">
          <router-link to="/login" class="default-link">
            <el-space>
              <span>{{ $t("pld.login") }}</span>
...
import {getUsers, SendEmail, Register} from "@/api/user";
...
const validatePasswd2 = (rule, value, callback) => {
  if (value === "") {
    ...
    callback(new Error(proxy.$t("pld.InvalidConfirmPasswd")));
  } else {
    callback();
  }
};
...
let sendSignUpEmail = async () => {
  ...
  const data = {
    to: formInline.email,
    subject: proxy.$t("pld.signupEmailSubject"),
